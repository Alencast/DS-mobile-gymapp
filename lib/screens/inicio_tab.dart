  ...
  void adicionarTreino() {
    if (tituloController.text.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Digite um título para o treino.")),
      );
      return;
    }
    Provider.of<TreinoProvider>(context, listen: false).adicionarTreino(
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text("Treino cadastrado com sucesso!")),
    );
            ...
            children: [
              TextField(
                ...
                onChanged: (valor) {
                  setState(() {
                    if (valor.length < 3) {
                      mensagemValidacao = "O título deve possuir pelo menos 3 caracteres.";
                    } else {
              ...
              Text(
                mensagemValidacao,
                style: TextStyle(
                  color: mensagemValidacao.contains("válido") ? Colors.green : Colors.red,
                ),
              ),
              ...
              Row(
                children: [
                  Expanded(child: ElevatedButton(onPressed: adicionarTreino, child: const Text('Adicionar Treino'))),
                  const SizedBox(width: 10),
                  Expanded(child: ElevatedButton(onPressed: limparCampos, child: const Text('Limpar'))),
                ],
              ),
            ],
          ),
                ...
                itemBuilder: (context, index) {
                  ...
                  return Padding(
                    ...
                    child: GestureDetector(
                      onTap: () {
                        // Navega para detalhe dentro do Navigator da aba (rota '/inicio/detalhe')
                        Navigator.of(context).pushNamed(
                          '/inicio/detalhe',
                          arguments: {'id': treino['id'], 'index': index},
                        );
                      },
                      onLongPress: () {
                      ...
                      child: Card(
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              Text(treino['titulo'] ?? '', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
  ...
  void _confirmarExclusao(BuildContext context, int index) {
    ...
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text("Deseja remover o treino '${provider.treinos[index]['titulo']}'?"),
        actions: [
